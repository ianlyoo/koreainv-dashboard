...
| 거래내역 | 등록 계좌 통합/개별 국내·해외 거래내역, KIS 실현손익, 최대 1년 조회 |
...
- 거래내역은 기본적으로 모든 등록 계좌를 통합하며 웹·Android에서 계좌별로 전환할 수 있다. 토스증권은 종료 주문의 체결내역을 표시하지만 Open API가 실현손익을 제공하지 않아 통합 손익과 세금 추정에서 제외됨을 화면에 명시한다.
