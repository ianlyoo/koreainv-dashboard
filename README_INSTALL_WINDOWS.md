...
이 문서는 Python이 설치되어 있지 않은 Windows PC에서 KISDashboard를 실행하는 방법을 안내합니다.
...
2. 기본 브라우저가 열리면 `http://127.0.0.1:8000` 대시보드로 이동합니다.
...
- 같은 포트를 쓰는 기존 프로세스가 실행 중일 수 있습니다.
- 기존 프로세스를 종료한 뒤 다시 실행합니다.

### 방화벽 경고 팝업이 뜰 때

- 로컬 실행이므로 허용해도 됩니다.
- 외부 네트워크 공개 용도는 권장하지 않습니다.

### 실행 직후 종료될 때

- `%APPDATA%\\KISDashboard\\logs\\launcher.log` 내용을 확인합니다.
- 로그를 첨부해 이슈를 등록하면 원인 파악이 빠릅니다.

### 업데이트 실패 시

- `%APPDATA%\\KISDashboard\\logs\\updater.log` 내용을 확인합니다.
- 앱을 완전히 종료한 뒤 최신 `KISDashboard-win64.zip`을 수동으로 덮어씌웁니다.
